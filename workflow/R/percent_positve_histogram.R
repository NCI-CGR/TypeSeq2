#'
percent_positive_histogram <- function(df, output_prefix = NULL) {
  if (nrow(df) == 0) {
    return(data.frame())
  }
  ...
  percentPositveHistrogramDf = df %>%
    ...
    do({
      ...
      plotCount = ggplot(temp, aes(hpvType, posCount, fill = factor(riskStatus))) +
        ...
        labs(
          title = paste0("Number of Samples Positive by Type for Project -- ", project_id),
          x = "hpv type",
          y = "sample count"
        ) +
        theme(
      ...
      plotPercent = ggplot(temp, aes(hpvType, percPos, fill = factor(riskStatus))) +
        ...
        labs(
          title = paste0(
            "Percent of Samples Positive by Type for Project -- ",
            project_id
          ),
          x = "hpv type",
          y = "percent positive"
        ...
        scale_fill_manual(values = pal_futurama(alpha = 0.8)(9)[2:9]) +
        geom_text(
          aes(label = round(100 * percPos, digits = 1)),
          hjust = 0.5,
          vjust = -0.25,
          size = 2.75
        ) +
  ...
  if (!is.null(output_prefix)) {
    percentPositveHistrogramDf %>% write_csv(sprintf(
      "%s.types_positive_histogram_plot_data.csv",
      output_prefix
    ))
  }
  
  
  return(percentPositveHistrogramDf)
}
