lineage_plot <- function(df, whichPlot) {
  ...
  lineage_plot_table = df %>%
    ...
    ungroup %>% mutate(HPV_Type = .sort_ids(HPV_Type),
                       Lineage_ID = .sort_ids(Lineage_ID))
  
  returnPlot1 = ggplot(lineage_plot_table,
                       aes(x = Lineage_ID, y = typeCount, fill = lineage_num)) +
    ...
    labs(
      title = "Lineage Distribution By Type.  Colors Represent Within Type Groupings",
      x = "Type",
      y = "Count",
      size = 12
    ) +
  ...
  returnPlot2 = ggplot(lineage_plot_table,
                       aes(x = HPV_Type, y = typeCount, fill = lineage_num)) +
