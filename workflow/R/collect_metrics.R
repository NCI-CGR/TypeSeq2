...
collect_metrics <- function(settings_lst, samples_only_for_report, detailed_pn_matrix_for_report, control_for_report, metrics_source_dir="./raw_metrics"){
    metrics <- list()
    ...
    read_summary <- read.csv("read_summary.csv") %>% setNames(c("id", "total", "pass_mapq", "pass_hamming",  "pass_mapq_perc", "pass_hamming_perc"))
    ...
    metrics$Perc_Reads_Filtered_MAPQ <- read_summary %$% { (sum(total) - sum(pass_mapq)) / sum(total)}
    ...
    # samples <- read.csv(samples_only_for_report_fn)
    ...
    t1 <- samples_only_for_report %>% group_by(Assay_Batch_Code) %>% 
        summarize(  n=n(), 
    ...
    specimen_control_defs = settings_lst$control_definitions
