...
#' take subset by batch; use barcodes if Assay_Batch_Code unavailable
#' @param x a float number between 0 and 1
#' @NoRd
...
plot_plate <- function(df, color_var, custom_color, title){
    ...
    dat2 <- df %>%   
        ...
        mutate(Control_Code = factor(
          ifelse(is.na(Control_Code),"empty",Control_Code), 
          levels=c("empty","control","sample") )
        )
    
    p <- dat2 %>% ggplot(aes(x = fct_reorder(colnum,sort(as.numeric(colnum))),y = fct_reorder(rownum,desc(rownum)),shape = Control_Code)) + 
      geom_point(aes_string(col = color_var), size =12, show.legend = T) +
      scale_shape_manual(name="Control Code", values = c("empty"=16,"control"=17,"sample"=16), limits=c("empty","control","sample"),  drop = F) +
      custom_color + 
...
save_to_csv <- function(dat, file=fn){
  write_csv(dat, file=file)
  return(file)
}

save_to_csv2 <- function(dat, file=fn){
