signal_to_noise_plot <- function(read_count_matrix_report,
  ...
  signalNoiseDf1 = read_count_matrix_report %>%
    ...
    inner_join(
      detailed_pn_matrix_for_report %>% filter(overall_qc == "pass") %>% gather(HPV_Type, hpvStatus, starts_with("HPV")),
      by = c("barcode", "HPV_Type", "Owner_Sample_ID")
    ) %>%
    filter(HPV_Type
           != "B2M") %>%
  ...
  df2 = signalNoiseDf1 %>%
    ...
    # find mean!
    summarize(meanCount = mean(HPV_Type_count, na.rm = T), .groups="drop") %>%
  ...
  df3 <- df2 %>%
    ...
    select(-temp) %>%
    mutate(hpvNum = ifelse(HPV_Type == "B2M_S2" |
                             HPV_Type == "B2M_S", 1, hpvNum)) %>%
  ...
  returnPlot = ggplot(signalNoiseDf,
                      aes(
                        x = HPV_Type,
                        y = meanCount,
                        color = hpvStatus,
    ...
    labs(
      title = "Signal to Noise for Current Ion Torrent Run",
      x = "HPV Types",
      y = "Average Counts",
      size = 18
    ) +
    ...
    theme(
      ...
      name = "Mean of the read counts",
      breaks = c("Amin_reads", "Bpos", "Cneg"),
      labels = c(
        "Positive call threshold",
        "Avg of bottom 10 positive samples",
        "Avg of top 10 negative samples"
      )
    ) +
    theme(
