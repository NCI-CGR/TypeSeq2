...
plate_summary <- function(samples_only_for_report, is_clinical=F,for_batch=F){
    ...
    t3 <- samples_only_for_report %>% group_by(!!vv, Assay_Plate_Code) %>% summarise( 
        hpv_pos_perc = fmt_perc(sum(Num_Types_Pos >0, na.rm=T) / n(), digits=1 ),
        plate_total_reads = scales::comma(sum(total_reads, na.rm = TRUE)),
        number_of_samples = n(), 
        num_samples_failed = sum(human_control == "failed_to_amplify", na.rm=T)
    ) %>% dplyr::select(!!vv, `Assay Plate Code` = Assay_Plate_Code,
